...
class BuildChainAgent:
    ...
    def create_retriever_chain(self):
        ...
        doc_prompt = ChatPromptTemplate.from_messages([
            ("system",
             "现在你是多智能体系统中的一个智能体，你的角色是软件开发过程中的:" + self.role + "请在你的指着视角解读你接受到的需求文本如下：{context}"),
            MessagesPlaceholder(variable_name="chat_history"),
            ("user", "{input}")
        ])
        ...
        template_second = "你的职责是:" + self.duty + "根据{role_text}的内容，给出你的设计实现方案。保持 json 格式输出"
    ...
    def process(self, input: str) -> str:
        prompt_str_input = self.prompt_template.format(description=self.description, input=input,
                                                       role=self.role)
