...
class BuildChainAgent:
    ...
    def __init__(self, role: str, duty: str, description: str):
        ...
        self.prompt_template_str = """
                    ...
                    - **重要**：回答中尽量避免使用‘#’等特殊字符。分点作答时使用markdown格式中的x标题来标注。
                    - **重要**：与其他角色对话时，保持现实世界中团队开发交流的口吻。
                    
                    ### 任务响应
                    - **非常重要**：对外界提供的基础信息进行分析，针对用户或其他角色提出的需求‘{input}’给出最完美的回答。
                    - 格式必须严格按照markdown格式输出，回答使用中文。
                    - 回答中应使用“我”来表达，按需换行，字数尽可能在2000字左右。
                    - **警告**：回答时禁止重复上一轮的回答，确保内容的新颖性和相关性。
                    
                    ### 角色具体要求
                    1. **角色明确**：清楚自己在对话中的角色和职责，确保回答符合角色的职能和任务。
                    2. **对话细节**：在对话中，始终保持角色一致性，生成生动的对话口吻。
                    3. **任务执行**：根据角色的职责，生成详尽的回答，确保符合软件开发流程。
                    
                    请根据这些要求生成您的回答，并确保所有输出均符合上述规则和格式要求。
