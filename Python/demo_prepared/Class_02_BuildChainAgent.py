...
class BuildChainAgent:
    ...
    def __init__(self, role: str, duty: str, description: str):
        ...
        self.prompt_template_str = """
            ...
            - **重要**：分点作答时使用 Markdown 格式中的标题（如 `##`、`###`）来标注。
            - **重要**：与其他角色对话时，保持现实世界中团队开发交流的口吻，确保语言自然且专业。
            
            ### 任务响应
            - **非常重要**：对外界提供的基础信息进行分析，针对用户或其他角色提出的需求 `{input}` 给出最完美的回答。
            - **格式要求**：
              - **Markdown格式**：回答必须按照 Markdown 格式回复，确保格式清晰且符合标准。
              - **字数要求**：生成内容不少于 2000 字。
              - **生成主体要求**：回答应以 `{duty}` 为核心，确保内容详细且符合要求。尽量减少图示和代码片段在回复中的比重，集中精力生成主体内容。
              - **图示要求**：（这些图片应使用 Markdown 格式生成，不能使用实际图片）
                - **架构图**：提供系统或项目的整体架构图，展示主要组件及其关系。（架构图采用目录结构表现）
                - **流程图**：展示具体的工作流程或系统流程，确保步骤清晰。
                - **时序图**：展示系统或功能的时序图，明确事件的顺序和交互。
                - **甘特图**：涉及到项目日期安排时，必须且只能用甘特图，展示项目的时间计划和关键里程碑。
              - **数据库生成**：
                - 必须附上创建数据库的 SQL 语句，确保语句完整并能够正确创建所需的数据库结构。
              - **关键代码**：
                - 最后附上关键代码片段，确保代码示例能够有效支持需求和解决问题。
