...
from SERVER import process_uploaded_file
...
def supervisor_chain(state: AgentState, conditional_map: Dict[str, Any]):
    ...
    if next_value in conditional_map.values():
        next_key = list(conditional_map.keys())[
            list(conditional_map.values()).index(next_value)
        ]
    else:
        next_key = "Finish"
...
async def websocket_run_workflow(websocket: WebSocket):
    ...
    try:
        ...
        await websocket.send_text(
            json.dumps(
                {"message": "Work Finished! Please checkout ", "role": "Bot2"},
                ensure_ascii=False,
            )
        )
