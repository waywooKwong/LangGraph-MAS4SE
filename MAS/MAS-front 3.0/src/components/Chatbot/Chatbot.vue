  ...
  <div class="joint-scope chatbot">
    ...
    <div class="side-bar">
      <div class="toggle-bar">
        ...
        <div
          @click="sendJsonToServer()"
          class="icon commit-user-design"
...
export default class Chatbot extends Vue {
  ...
  private websocket: WebSocket | null = null; // 添加WebSocket属性
  private subscriptions = new Subscription();

  public mounted(): void {
    const {
      $el,
      $refs: { paper, stencil, toolbar },
      subscriptions,
      $eventBusService,
    } = this;
    subscriptions.add(
      $eventBusService.subscribe(SharedEvents.GRAPH_CHANGED, (json: Object) =>
        this.onJointGraphChange(json)
      )
    );
    subscriptions.add(
      $eventBusService.subscribe(
        SharedEvents.JSON_EDITOR_CHANGED,
        (json: Object) => this.onJsonEditorChange(json)
      )
    );
  ...
  public async sendJsonToServer(): Promise<void> {
    ...
    try {
      ...
      const jsonBlob = new Blob([JSON.stringify(this.fileJSON)], {
